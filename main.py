...
def getService():
    ...
    # Print message to get the pin
    print(jsonLoader.takeDataFromJson("messages", "getPin"))
    ...
    # Check if pin == to pin of settings.json
    pinChecker = jsonLoader.checkPin(pin)
    ...
    if pinChecker == 100:
        ...
        # Print how to get service methods
        print(jsonLoader.takeDataFromJson("messages", "getServiceMethod"))

        # Methods selector input
        opt = str(int(input("\n" + username + "@option" + inputTemplate)))
        ...
        if opt == "1":
            ...
            # Name of the service input
            service = str(input("\n" + username + "@service" + inputTemplate))

            # Get value of getPasswords at data
            data = jsonLoader.getPassword(service=service)
            clearTerm()  # Clear term/console
            ...
            if data[0] == 102:
                if data[1] == 105:  # Idk what it checks but it's working
                    ...
                    for i in range(len(data[2])):
                        # Pattern
                        print(
                            f"- {1+i}) Service: {service}, Email: {data[2][i][0]}, Password: {data[2][i][1]}")
        ...
        # Check if user typed 2 and do
            ...
            email = str(input("\n" + username + "@email" +
                              inputTemplate))  # Email selector

            # Get response of method to data var
            data = jsonLoader.getPassword(email=email)
            clearTerm()  # Clear term/console
                ...
                if data[1] == 105:  # Idk but it's working
                    ...
                    for i in range(len(data[2])):
                        for k in data[2][i]:  # Get service name
                            ...
                            # Message pattern
                            print(
                                f"- {1+i}) Service: {list(data[2][i].keys())[0]}, Email: {data[2][i][k][0]}, Password: {data[2][i][k][1]}")
        ...
        # If user typed 3 do:
            ...
            data = jsonLoader.getPassword(
                method="all")  # Get response to data var
                ...
                if data[1] == 105:  # Idk what i check, but it's working
                    ...
                    for i in range(len(data[2])):
                        ...
                        for k in data[2][i]:
                            ...
                            # Message pattern
                            print(
                                f"- {1+i})Service: {list(data[2][i].keys())[0]}, Email: {data[2][i][k][0]}, Password: {secure.decryptData(data[2][i][k][1])}")
                    print("\n#----------------------------------------#")
...
def setService():
    ...
    service = str(input("\n" + username + "@service" +
                        inputTemplate))  # Set service
    email = str(input(username + "@email" + inputTemplate))  # Set email
    password = str(input(username + "@password" +
                         inputTemplate))  # Set password

    setService = jsonLoader.setService(
        service, email, password)  # Set service credentials
...
def getStarted():
    # Print message where you can choice the action
    print(jsonLoader.takeDataFromJson('messages', 'getStarted'))
    ...
    if initInput == "register" or initInput == "/register":
        clearTerm()  # Clear term/console
        print("\n" + jsonLoader.takeDataFromJson('messages',
                                                 'startRegister'))  # Print message
        setService()  # Callback
...
if startExecution == "start" or startExecution == "/start":
    ...
    if jsonLoader.checkFristExec() == "true":
        # Print message to insert a name and a pin
        print("\n" + jsonLoader.takeDataFromJson('messages', 'pinInsert'))
        ...
        # With this line i set the name, pin and the execution. So firstExec = false beacouse it's started
        jsonLoader.setExecution(name, pin)
        # Send message where credentials will be setted correctly
        print(jsonLoader.takeDataFromJson('messages', 'credentialSuccesSetted'))
        clearTerm()  # Clear terminal console
        # Take user name from getNameFromJson method in jsonLoader
        username = jsonLoader.getNameFromJson()
